...
def compute_vmaf(orig_path, recon_path):
    ...
    try:
        ...
        match = re.search(r"VMAF score: ([\d\.]+)", res.stderr)
        if match:
...
def compute_detection_accuracy(orig_path, recon_path):
    try:
        from ultralytics import YOLO
        model = YOLO("yolov8n.pt")
        res_orig = model(orig_path, verbose=False)
        res_recon = model(recon_path, verbose=False)
        count_orig = len(res_orig[0].boxes)
        count_recon = len(res_recon[0].boxes)
        if count_orig == 0:
            return 1.0 if count_recon == 0 else 0.0
        return min(count_recon / count_orig, 1.0)
    except Exception as e:
        print("Detection accuracy error:", e)
        return None
...
def run_experiment(bg_quality, roi_quality):
    ...
    # VMAF and Detection Accuracy
    vmaf_score = compute_vmaf(orig_local, recon_local)
    det_acc = compute_detection_accuracy(orig_local, recon_local)
    ...
    row = {
        ...
        "vmaf": vmaf_score if vmaf_score is not None else "",
        "det_acc": det_acc if det_acc is not None else "",
        "saved_dir": saved_dir
    }
...
def main():
    ...
    fieldnames = [
        ...
        "bg_psnr","bg_ssim","vmaf","det_acc","saved_dir"
    ]
